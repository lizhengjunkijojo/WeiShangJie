...
public class NewsDetailFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        mDetail = (News) getActivity().getIntent().getSerializableExtra(BUNDLE_KEY_TYPE);
    ...
    private void requestData(News detail) {
    ...
    private class CacheTask extends AsyncTask<String, Void, News> {
        ...
        @Override
        protected void onPostExecute(News detail) {
    ...
    //执行加载成功的数据显示
    protected void executeOnLoadDataSuccess(News detail) {
    ...
    protected void saveCache(News detail) {
    ...
    protected String getCacheKey() {
        return "freeDetail_" + mDetail.title;
    }

    /**
     * 摘要
     */
    protected void getHeadViewBody(News detail) {
        ll_header.setVisibility(View.GONE);

        if(!TextUtils.isEmpty(detail.title)){
            tv_title.setText(detail.title);
        }
        if(!TextUtils.isEmpty(detail.srcfrom)){
            tv_source.setText(detail.srcfrom);
        }
    ...
    protected String getWebViewBody(News detail) {
        ...
        if(!TextUtils.isEmpty(detail.kuaixunContent)){

            body.append(UIHelper.WEB_STYLE).append(UIHelper.WEB_LOAD_IMAGES);
            body.append(UIHelper.WebViewBodyString);
            byte b[]=android.util.Base64.decode(detail.kuaixunContent,Base64.DEFAULT);
